...
//	env.positional_arg('n',"names",  names_name,      "please provide a filename.", "names of individuals in the sample.");
	...
	Matrix<TYPE, 7, 1> tr=MATRIX::Zero(7,1);
	for(int z=0; z<num_blocks; z++)
	{
		MATRIX MJM=r[z].Mt1*r[z].Mt1.transpose();
		MATRIX HJH=r[z].Ht1*r[z].Ht1.transpose();
		MATRIX MJH=(r[z].Mt1*r[z].Ht1.transpose()+r[z].Ht1*r[z].Mt1.transpose() )/2;

		size_t n=r[z].MtM.rows();

		tr(0,0)+=r[z].MtM.trace();
		tr(1,0)+=MJM.trace();
		tr(2,0)+=r[z].HtH.trace();
		tr(3,0)+=HJH.trace();
		tr(4,0)+=r[z].MtH.trace();
		tr(5,0)+=MJH.trace();
		tr(6,0)+=n;
	}
...

	{
		...
		TYPE var=TYPE( (y.adjoint() * y)(0,0) ) / TYPE(N - 1.);
	...
	MATRIX K=MATRIX::Zero(5,1);

	std::cerr << "lm: " << t( o(sub5(h, tr), norm) ) << std::endl;
	std::cout << "Un-normed : " << t(sub5(h, tr) ) << std::endl;
	std::cout << "LL: " << get_ll(h, p[0], r[0], trait_num) << std::endl;
	std::cerr << "vars: " << t(qv(sub5(h, tr), var_norm) ) << std::endl;

	std::cerr << get_Sigma(h, r[0]).trace() << std::endl;
	...
	if (!hessian)
	{
		...
		t1=clock();
		std::cerr << x << ", " << t1-t0 << ", " << f_n.squaredNorm() << "\nh: " << t( o(sub5(h_o, tr), norm) ) << std::endl << std::endl;
		...
		MATRIX iH=MATRIX::Zero(5,5);
		MATRIX j=MATRIX::Zero(5,1);

		for (int z=0; z<num_blocks; z++)
		{
			std::cerr << __LINE__ << std::endl;
			pair2 q=nablas(h, p[z], r[z], trait_num);	
			std::cerr << "(" << q.H.rows() << "x" << q.H.cols() << ")" << std::endl;
			iH+=q.H;
			j+=q.j;
		}
		...
		t1=clock();
		std::cerr << x << ", time:" << t1-t0 << ", norm(j):" << j.squaredNorm() << std::endl << "j:" << t(j) << std::endl << "h:" << t( o(sub5(h, tr), norm) ) << std::endl << std::endl;	
		std::cout << "LL:" << get_ll(h, p[0], r[0], trait_num) << std::endl;
	...
	MATRIX v0=qv(h_n-VECTOR(ARRAY1(H_n.diagonal()).abs().sqrt())*2, var_norm);
	MATRIX v1=qv(h_n, var_norm);
	MATRIX v2=qv(h_n+VECTOR(ARRAY1(H_n.diagonal()).abs().sqrt())*2, var_norm);
	...
	std::cout << "h0 : " << t(o(sub5(h_n-VECTOR(ARRAY1(H_n.diagonal()).abs().sqrt())*2, tr ), norm) ) << std::endl;
	std::cout << "h1 : " << t(o(sub5(h_n, tr), norm) ) << std::endl;
 	std::cout << "h2 : " << t(o(sub5(h_n+VECTOR(ARRAY1(H_n.diagonal()).abs().sqrt())*2, tr ), norm) ) << std::endl;
	std::cout << "Un-normed : " << t(sub5(h_n, tr) ) << std::endl;

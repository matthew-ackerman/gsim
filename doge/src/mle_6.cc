...
//	exit(0);
	...
	bool hessian=false, block=false, skip=false;
	int trait_num;
	int num_blocks=1;
	TYPE loci=0;
	print_options();
	...
	env.flag(	's',"skip", &skip,	&flag_set, 	"an error occurred while displaying the version message", "skip maximization");		//DONE
...
#define NORM
	...
	std::cout << r[0].MtM.trace() << std::endl;
	...
	std::cerr << "h is: " << t(sub(h)) << std::endl;
...
/*	
	...
	if (h(2,0)<0 )
		h(2,0)=fabs( h(2,0) );
	if (h(4,0) > std::min (h(0,0), h(2,0) ) )
		h(4,0)= std::min (h(0,0), h(2,0) ) ;
	std::cerr << "better: " << t(sub(h)) << std::endl;
*/
	/*h(0,0)=0.000015;
	h(1,0)=0.1;
	h(2,0)=0.000015;
...
//		std::cout << "LL:" << get_ll(h_n, p[0], r[0], trait_num) << std::endl;
		...
		std::cerr << "Rho: " << qv(h, var_norm)(4,0)/sqrt(qv(h, var_norm)(0,0)*qv(h,var_norm)(2,0) )  << std::endl;
	...
	std::cout << "var(Z) : " << norm(0,0) << std::endl;
